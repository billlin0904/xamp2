...
void DSPManager::RemovePostDSP(Uuid const& id) {
    std::remove_if(setting_post_dsp_chain_.begin(),
        setting_post_dsp_chain_.end(),
        [id](auto const& processor) {
            return processor->GetTypeId() == id;
        });
...
void DSPManager::RemovePreDSP(Uuid const& id) {
    std::remove_if(setting_pre_dsp_chain_.begin(),
        setting_pre_dsp_chain_.end(),
        [id](auto const& processor) {
            return processor->GetTypeId() == id;
        });
...
void DSPManager::SetReplayGain(float volume) {
    replay_gain_ = volume;
}

bool DSPManager::IsEnableDSP() const {
...
bool DSPManager::IsEnableSampleRateConverter() const {
    return GetPreDSP<SoxrSampleRateConverter>() != nullptr
        || GetPostDSP<SoxrSampleRateConverter>() != nullptr;
}
...
bool DSPManager::ProcessDSP(const float* samples, uint32_t num_samples, AudioBuffer<int8_t>& fifo) {
    if (CanProcessFile()) {
        if (ApplyDSP(samples, num_samples, fifo)) {
            return true;
        }
    }
    else {
        converter_->Process(samples, num_samples, fifo);
    }
    return false;
}

bool DSPManager::ApplyDSP(const float* samples, uint32_t num_samples, AudioBuffer<int8_t>& fifo) {
    auto* soxr = GetPreDSP<SoxrSampleRateConverter>();

    if (soxr != nullptr) {
        auto temp = MakeBuffer<float>(num_samples);
        if (!soxr->Process(samples, num_samples, temp)) {
            return true;
        }
        for (auto i = 1; i < pre_dsp_.size(); ++i) {
            pre_dsp_[i]->Process(temp.data(), temp.size(), dsp_buffer_);
        }
    } else {
        for (auto i = 0; i < pre_dsp_.size(); ++i) {
            pre_dsp_[i]->Process(samples, num_samples, dsp_buffer_);
        }
    }
    ...
    if (!converter_->Process(dsp_buffer_, fifo)) {
        return true;
    }
    return false;
}
...
void DSPManager::InitDsp(AudioFormat input_format, AudioFormat output_format, DsdModes dsd_mode, uint32_t sample_size) {
    ...
    if (auto* soxr = GetPreDSP<SoxrSampleRateConverter>()) {
        soxr->Init(input_format.GetSampleRate());
        XAMP_LOG_D(logger_, "Init soxr input smaple rate: {}.", input_format.GetSampleRate());
    }

    if (auto* volume = GetPreDSP<BassVolume>()) {
        volume->Init(replay_gain_);
        XAMP_LOG_D(logger_, "Set replay gain: {} db.", replay_gain_);
    }

    if (auto* eq = GetPreDSP<IEqualizer>()) {
