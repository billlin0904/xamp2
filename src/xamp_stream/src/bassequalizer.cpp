...
public:
    BassEqualizerImpl()
		: preamp_(0) {
        logger_ = XAMP_LOG_CREATE_LOGGER(BassEqualizer);
    }
    ...
    void AddBand(uint32_t i, float freq, float band_width, float gain, float Q) {
        const auto fx_handle = BassLibDLL.BASS_ChannelSetFX(impl_.get(), BASS_FX_BFX_PEAKEQ, 1);
        ...
        BassIfFailedThrow(BassLibDLL.BASS_FXSetParameters(fx_handle, &eq));
    ...
    void SetEq(uint32_t band, float gain, float Q) {
        ...
        BASS_BFX_PEAKEQ eq{};
        BassIfFailedThrow(BassLibDLL.BASS_FXGetParameters(fx_handles_[band], &eq));
        eq.fGain = gain;
        eq.fQ = Q;
        BassIfFailedThrow(BassLibDLL.BASS_FXSetParameters(fx_handles_[band], &eq));
    }
