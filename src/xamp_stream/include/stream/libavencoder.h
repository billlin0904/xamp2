...
public:
	...
	void Start(const Property& config, const std::shared_ptr<FastIOStream>& file) override;

	void Encode(std::function<bool(uint32_t)> const& progress = nullptr, const std::stop_token& stop_token = std::stop_token()) override;
