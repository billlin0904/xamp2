...
inline constexpr std::uint64_t kXoshiroDefaultSeed = 1234567890ULL;

static inline /*constexpr*/ uint64_t Rotl(const uint64_t x, int32_t k) noexcept {
    return (x << k) | (x >> (64 - k));
}

template <size_t N>
std::array<uint64_t, N> Splitmix64(uint64_t state) noexcept {
