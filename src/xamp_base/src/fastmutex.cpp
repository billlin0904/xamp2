...
static XAMP_ALWAYS_INLINE uint32_t ToMilliseconds(const timespec* ts) {
	return ts->tv_sec * 1000 + ts->tv_nsec / 1000000;
}

template <typename T>
static XAMP_ALWAYS_INLINE bool PlatformFutexWait(std::atomic<T>& to_wait_on, uint32_t &expected, uint32_t milliseconds) {
#ifdef XAMP_OS_WIN
...
#else
	static_assert(false, "Not Support on this platform");
#endif
}

template <typename T>
static XAMP_ALWAYS_INLINE void PlatformFutexWakeSingle(std::atomic<T>& to_wake) {
#ifdef XAMP_OS_WIN
...
template <typename T>
static XAMP_ALWAYS_INLINE void PlatformFutexWakeAll(std::atomic<T>& to_wake) {
#ifdef XAMP_OS_WIN
...
int FastConditionVariable::FastWait(std::atomic<uint32_t>& to_wait_on, uint32_t expected, const timespec* to) noexcept {
#if defined(XAMP_OS_WIN) || defined(XAMP_OS_MAC)
	if (to == nullptr) {
