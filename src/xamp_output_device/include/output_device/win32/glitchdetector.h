...
public:
	...
	void Reset(uint64_t device_frequency, Duration buffer_duration) noexcept;

	std::optional<AudioGlitchInfo> Update(uint64_t position, uint64_t qpc_position_100ns) noexcept;

	AudioGlitchInfo GetTotalGlitchInfoAndReset() noexcept;
...
inline void GlitchDetector::Reset(uint64_t device_frequency, Duration buffer_duration) noexcept {
...
inline std::optional<AudioGlitchInfo> GlitchDetector::Update(uint64_t position, uint64_t qpc_position_100ns) noexcept {
	// 尚未初始化或參數異常就直接跳過
...
inline AudioGlitchInfo GlitchDetector::GetTotalGlitchInfoAndReset() noexcept {
