...
namespace {
    ...
    void GetChannelPeaksAndRms(const std::vector<float>& buffer,
        size_t frame_per_peek,
        std::vector<float>& left_peaks,
        std::vector<float>& right_peaks,
        std::vector<float>& left_rms,
        ...
        // buffer.size() 為 float 數量，每 2 個 float 為 (L, R) 一組
        const size_t frame_count = buffer.size() / 2;
        ...
        for (size_t start = 0; start < frame_count; start += frame_per_peek) {
            ...
            float sum_sq_left = 0.f;
            float sum_sq_right = 0.f;
            size_t count = end - start;

            // 逐取樣計算 Peak 與 RMS
            for (size_t f = start; f < end; f++) {
                float l = std::fabs(buffer[f * 2 + 0]);
                float r = std::fabs(buffer[f * 2 + 1]);
            ...
            float rms_right = 0.f;
            if (count > 0) {
                rms_left = std::sqrt(sum_sq_left / static_cast<float>(count));
                rms_right = std::sqrt(sum_sq_right / static_cast<float>(count));
            }
