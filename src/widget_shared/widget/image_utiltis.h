...
XAMP_WIDGET_SHARED_EXPORT std::vector<uint8_t> Pixmap2ByteVector(const QPixmap& source);

XAMP_WIDGET_SHARED_EXPORT QByteArray Pixmap2ByteArray(const QPixmap& source);
...
XAMP_WIDGET_SHARED_EXPORT void OptimizePng(const QByteArray& buffer, std::vector<uint8_t>& result_png);
