...
struct XAMP_WIDGET_SHARED_API ConstexprQString final : public QLatin1String {
...
XAMP_WIDGET_SHARED_API inline bool isNullOfEmpty(const QString& s) {
	return s.isNull() || s.isEmpty();
}

XAMP_WIDGET_SHARED_API inline QString qFormat(const QString &str) {
	return str;
}

XAMP_WIDGET_SHARED_API inline QString qFormat(char const* const str) {
    return {QLatin1String{ str }};
}

XAMP_WIDGET_SHARED_API QString formatSampleRate(uint32_t sample_rate);

XAMP_WIDGET_SHARED_API QString formatBitRate(uint32_t bit_rate);

XAMP_WIDGET_SHARED_API QString colorToString(QColor color);

XAMP_WIDGET_SHARED_API QString formatDuration(const double stream_time, bool full_text = false);

XAMP_WIDGET_SHARED_API QString formatDurationAsMinutes(const double stream_time);

XAMP_WIDGET_SHARED_API bool isMoreThan1Hours(const double stream_time);

XAMP_WIDGET_SHARED_API QString toNativeSeparators(const QString& path);

XAMP_WIDGET_SHARED_API bool parseVersion(const QString& s, QVersionNumber& version);

XAMP_WIDGET_SHARED_API QString formatVersion(const QVersionNumber& version);
