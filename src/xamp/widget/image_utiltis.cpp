...
inline bool operator ==(Float3 const& value1, Float3 const& value2) noexcept {
...
ImageColorAnalyzer::ImageColorAnalyzer(QImage const &image) {	
	Analyze(image);
}

void ImageColorAnalyzer::Analyze(QImage const &image) {
	// Convert to HCL color spec image.
	// Count of HCL color in image. 	
	std::map<Float3, int> stats;

	for (auto i = 0; i < image.width(); ++i) {
		for (auto j = 0; j < image.height(); ++j) {
			 auto hcl = RGBtoHCL(image.pixelColor(i, j));
			 stats[hcl]++;
		}
	}

	// Sort HCL color by count. 
	std::vector<std::pair<Float3, int>>
		result{ stats.begin(),stats .end() };
	...
	primary_color_ = HCLtoRGB(result.front().first);
	auto r = primary_color_.red();
	auto g = primary_color_.green();
	auto b = primary_color_.blue();

	if (result.size() > 1) {
		secondary_color_ = HCLtoRGB(result[1].first);
		r = secondary_color_.red();
		g = secondary_color_.green();
		b = secondary_color_.blue();
	}
}
