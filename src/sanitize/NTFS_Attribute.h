 ...
 * Modifications/Bug Fixes by Kelson <kelson@shysecurity.com>
 *
...
#ifndef NTFSLIB_ATTRIBUTE_HEADER
#define	NTFSLIB_ATTRIBUTE_HEADER
...
public:
	...
	virtual BOOL ReadData(const ULONGLONG &offset, void *bufv, ULONGLONG bufLen, ULONGLONG *actual) const = 0;
};	// CAttrBase
...
int CAttrBase::GetAttrName(char *buf, DWORD bufLen) const
{
	...
	if (bufLen < AttrHeader->NameLength) return -1*AttrHeader->NameLength;	// buffer too small
	...
	if(!len) return -1 * AttrHeader->NameLength;
...
{
	if(!AttrHeader->NameLength) return 0;
	if (bufLen < AttrHeader->NameLength) return -1*AttrHeader->NameLength;	// buffer too small
...
public:
	...
	virtual BOOL ReadData(const ULONGLONG &offset, void *bufv, ULONGLONG bufLen, ULONGLONG *actual) const;
};	// CAttrResident
...
// Number of bytes actually read is returned in "*actual"
BOOL CAttrResident::ReadData(const ULONGLONG &offset, void *bufv, ULONGLONG bufLen, ULONGLONG *actual) const
{
	_ASSERT(bufv);

	*actual = 0;
	if(bufv == 0) return FALSE;
	if(bufLen == 0) return TRUE;
	if(offset >= AttrBodySize) return FALSE;
	if((offset + bufLen) > AttrBodySize) *actual = AttrBodySize - offset;
	else                                 *actual = bufLen;
	memcpy(bufv, (BYTE*)AttrBody + offset, (size_t)*actual);
	return TRUE;
}
...
private:
	...
	BOOL ReadVirtualClusters(ULONGLONG vcn, DWORD clusters,
		void *bufv, DWORD bufLen, DWORD *actual);
...
BOOL CAttrNonResident::ReadClusters(void *buf, DWORD clusters, LONGLONG lcn)
{
	if (lcn == -1)	// sparse data
	{
		...
		memset(buf, 0, clusters * _ClusterSize); // fill the buffer with 0
		return TRUE;
	}
	...
	char *alignedBuf = (char*)buf;
	DWORD bytes = clusters*_ClusterSize;
	if(((ULONGLONG)buf) % _SectorSize) alignedBuf = new char[bytes]; // buffer must be aligned
	
	if(!ReadFile(_hVolume, alignedBuf, bytes, &len, NULL) || len != bytes)
	{
		NTFS_TRACE1("Cannot read cluster with LCN %I64d\n", lcn);
		if(alignedBuf != buf) delete alignedBuf;
		return FALSE;
	}
	NTFS_TRACE2("Successfully read %u clusters from LCN %I64d\n", clusters, lcn);
	if(alignedBuf != buf) { memcpy(buf,alignedBuf,bytes); delete alignedBuf; }
	return TRUE;
}
...
BOOL CAttrNonResident::ReadVirtualClusters(ULONGLONG vcn, DWORD clusters, void *bufv, DWORD bufLen, DWORD *actual)
{
	_ASSERT(bufv);
	_ASSERT(clusters);
	...
	*actual = 0;
	...
	while(dr)
	{
		if (vcn>=dr->StartVCN && vcn<=dr->LastVCN)
		{
			...
			if ((ULONGLONG)clusters > vcns)	clustersToRead = (DWORD)vcns; // Fragmented data, we must go on
			else                            clustersToRead = clusters;
			if (ReadClusters(buf, clustersToRead, dr->LCN+(vcn-dr->StartVCN)))
			{
				vcn += clustersToRead;
				clusters -= clustersToRead;
				buf += clustersToRead*_ClusterSize;
				*actual += clustersToRead*_ClusterSize;
			}
			else break;
			if (clusters == 0) break;
		}
...
// Read "bufLen" bytes from "offset" into "bufv"
// Number of bytes actually read is returned in "*actual"
BOOL CAttrNonResident::ReadData(const ULONGLONG &offset, void *bufv, ULONGLONG bufLen, ULONGLONG *actual) const
{
	...
	*actual = 0;
	...
	if((offset + bufLen) > AttrHeaderNR->RealSize) bufLen = (ULONGLONG)(AttrHeaderNR->RealSize - offset);

	DWORD len;
	BYTE *buf = (BYTE*)bufv;

	ULONGLONG startVCN = offset / _ClusterSize; // first cluster number
	ULONGLONG startOffset = offset % _ClusterSize;
	ULONGLONG startBytes = _ClusterSize - startOffset; // bytes in first cluster
	if(startBytes > bufLen) startBytes = bufLen;
	
	if(startBytes != _ClusterSize) // Read first cluster
	{
		...
		{
			memcpy(buf, UnalignedBuf + startOffset, (size_t)startBytes);
			buf += startBytes;
			bufLen -= startBytes;
			*actual += startBytes;
			startVCN++;
	...
	ULONGLONG alignedClusters = bufLen / _ClusterSize;
	if (alignedClusters)
	{
		// Aligned clusters
		ULONGLONG alignedSize = alignedClusters*_ClusterSize;
		if(((CAttrNonResident*)this)->ReadVirtualClusters((DWORD)startVCN, (DWORD)alignedClusters, buf, (DWORD)alignedSize, &len) && len == alignedSize)
		{
			buf += alignedSize;
			bufLen -= alignedSize;
			*actual += alignedSize;
	...
	ULONGLONG lastVCN = startVCN; // last cluster number
	ULONGLONG lastBytes = bufLen; // bytes in last cluster

	// Last cluster, Unaligned
	if (((CAttrNonResident*)this)->ReadVirtualClusters(startVCN, 1, UnalignedBuf, _ClusterSize, &len) && len == _ClusterSize)
	{
		memcpy(buf, UnalignedBuf, (size_t)bufLen);
		*actual += bufLen;
	}
	else return FALSE;
	return TRUE;
...
int CFileName::GetFileName(wchar_t *buf, DWORD bufLen) const
{
	...
	if(!FileName->NameLength) return 0;
	if(bufLen < FileName->NameLength) return -1*FileName->NameLength;
...
public:
	CAttr_VolName(const ATTR_HEADER_COMMON *ahc, const CFileRecord *fr) : CAttrResident(ahc, fr)
	{
		...
		WideCharToMultiByte(CP_ACP, 0, VolNameU, NameLength, VolNameA, NameLength, NULL, NULL);
...
public:
	...
	{
		if (len < NameLength) return -1*NameLength;
		wcsncpy_s(buf, NameLength+1, VolNameU, NameLength);
		return NameLength;
	}
	...
	{
		if (len < NameLength) return -1*NameLength;
		strncpy_s(buf, NameLength+1, VolNameA, NameLength);
		return NameLength;
	}
}; // CAttr_VolInfo
