...
#ifndef NTFSLIB_FILERECORD_HEADER
#define	NTFSLIB_FILERECORD_HEADER
...
private:
	...
	BOOL OpenVolume(wchar_t volume);
...
private:
	...
	BOOL VisitIndexBlock(const ULONGLONG &vcn, const wchar_t *fileName, CIndexEntry &ieFound) const;
...
public:
	...
	__inline const BOOL FindSubEntry(const wchar_t *fileName, CIndexEntry &ieFound) const;
	const CAttrBase* FindStream(wchar_t *name = NULL);
...
ULONGLONG CFileRecord::GetFileReference() const
{
	return FileReference & 0x0000FFFFFFFFFFFFUL;
}
...
FILE_RECORD_HEADER* CFileRecord::ReadFileRecord(ULONGLONG &fileRef)
{
	...
	if (fileRef < MFT_IDX_USER || Volume->MFTData == NULL)
	{
		...
		if (frAddr.LowPart == DWORD(-1) && GetLastError() != NO_ERROR) return FALSE;
		else
		{
			DWORD len;
			fr = (FILE_RECORD_HEADER*)new BYTE[Volume->FileRecordSize];
			if (ReadFile(Volume->hVolume, fr, Volume->FileRecordSize, &len, NULL) && len==Volume->FileRecordSize) return fr;
			else
			{
				delete fr;
				return NULL;
	...
	else
	{
		...
		if (Volume->MFTData->ReadData(frAddr, fr, Volume->FileRecordSize, &ulen)
			&& ulen == Volume->FileRecordSize)
			return fr;
		else
		{
			delete fr;
...
BOOL CFileRecord::VisitIndexBlock(const ULONGLONG &vcn, const wchar_t *fileName, CIndexEntry &ieFound) const
{
...
int CFileRecord::GetFileName(wchar_t *buf, DWORD bufLen) const
{
...
__inline const BOOL CFileRecord::FindSubEntry(const wchar_t *fileName, CIndexEntry &ieFound) const
{
...
const CAttrBase* CFileRecord::FindStream(wchar_t *name)
{
	const CAttrBase *data = FindFirstAttr(ATTR_TYPE_DATA);
	while (data)
	{
		if (data->IsUnNamed() && name == NULL) break;   // Unnamed stream
		if ((!data->IsUnNamed()) && name)               // Named stream
		{
			wchar_t an[MAX_PATH];
			if(data->GetAttrName(an, MAX_PATH))
				if (wcscmp(an, name) == 0) break;
		}
...
///////////////////////////////////////
CNTFSVolume::CNTFSVolume(wchar_t volume)
{
...
BOOL CNTFSVolume::OpenVolume(wchar_t volume)
{
	if(volume < wchar_t('a') || wchar_t('z') < volume)
		if(volume < wchar_t('A') || wchar_t('Z') < volume)
			return FALSE;

	wchar_t volumePath[7];
	wcsncpy_s(volumePath, L"\\\\.\\::", _TRUNCATE);
	volumePath[4] = volume;

	hVolume = CreateFile(volumePath, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
		NULL, OPEN_EXISTING, FILE_ATTRIBUTE_READONLY | FILE_FLAG_BACKUP_SEMANTICS, NULL);
	if (hVolume != INVALID_HANDLE_VALUE)
	{
		...
		if (ReadFile(hVolume, &bpb, 512, &num, NULL) && num==512)
		{
			if (strncmp((const char*)bpb.Signature, NTFS_SIGNATURE, 8) == 0)
			{
				...
				if (sz > 0) FileRecordSize = ClusterSize * sz;
				else        FileRecordSize = 1 << (-sz);
				NTFS_TRACE1("FileRecord Size = %u bytes\n", FileRecordSize);

				sz = (char)bpb.ClustersPerIndexBlock;
				if (sz > 0) IndexBlockSize = ClusterSize * sz;
				else        IndexBlockSize = 1 << (-sz);
