...
/*
 *
 * fftw_plan_guru64_dft - FFTW wrapper to Intel(R) oneAPI Math Kernel Library (Intel(R) oneMKL).
 *
 ******************************************************************************
 */
...
fftw_plan_guru64_dft_omp_offload(int rank, const fftw_iodim64 *dims, int howmany_rank,
                                 const fftw_iodim64 *howmany_dims, fftw_complex *in,
                                 fftw_complex *out, int sign, unsigned flags,
                                 void* interopObj)
{
    if(mkl_dfti_is_ilp64 == 0)
      return fftw_plan_guru64_dft_omp_offload_impl_lp64(rank, dims, howmany_rank, howmany_dims,
                                                   in, out, sign, flags, interopObj);
    else
      return fftw_plan_guru64_dft_omp_offload_impl_ilp64(rank, dims, howmany_rank, howmany_dims,
                                                   in, out, sign, flags, interopObj);
}

fftw_plan
fftw_plan_guru64_dft_omp_offload_impl_ilp64(int rank, const fftw_iodim64 *dims, int howmany_rank,
                                        const fftw_iodim64 *howmany_dims, fftw_complex *in,
                                        fftw_complex *out, int sign, unsigned flags,
                                        void* interopObj)
#else
fftw_plan
fftw_plan_guru64_dft_omp_offload_impl_lp64(int rank, const fftw_iodim64 *dims, int howmany_rank,
                                        const fftw_iodim64 *howmany_dims, fftw_complex *in,
                                        fftw_complex *out, int sign, unsigned flags,
                                        void* interopObj)
#endif // MKL_ILP64
