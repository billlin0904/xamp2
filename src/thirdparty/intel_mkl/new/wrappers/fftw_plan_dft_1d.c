...
/*
 *
 * fftw_plan_dft_1d - FFTW3 wrapper to Intel(R) oneAPI Math Kernel Library (Intel(R) oneMKL).
 *
 ******************************************************************************
 */
...
fftw_plan_dft_1d_omp_offload(int n, fftw_complex *in, fftw_complex *out,
                             int sign, unsigned flags, void* interopObj)
{
    return fftw_plan_dft_omp_offload_impl(1, &n, in, out, sign, flags,
                                            interopObj);
}
#else
fftw_plan
