...
/*
 *
 * fftw_plan_dft_3d - FFTW3 wrapper to Intel(R) oneAPI Math Kernel Library (Intel(R) oneMKL).
 *
 ******************************************************************************
 */
...
fftw_plan_dft_3d_omp_offload(int nx, int ny, int nz, fftw_complex *in, fftw_complex
                             *out, int sign, unsigned flags, void* interopObj)
{
    int n[3] = { nx, ny, nz };
    return fftw_plan_dft_omp_offload_impl(3, n, in, out, sign, flags, interopObj);
}
#else
fftw_plan
