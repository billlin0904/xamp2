...
/*
 *
 * dfftw_plan_guru_dft - FFTW3 Fortran 77 wrapper to Intel(R) oneAPI Math Kernel Library (Intel(R) oneMKL).
 *
 ******************************************************************************
 */
...
DLL_EXPORT void
dfftw_plan_guru_dft_cpu(PLAN *p, INTEGER *rank, INTEGER *n, INTEGER *is,
...
{
    dfftw_plan_guru_dft(p, rank, n, is, os, howmany_rank, howmany_n, howmany_is,
                        howmany_os, (COMPLEX16 *)in->base_addr,
                        (COMPLEX16 *)out->base_addr, sign, flags);
}

DLL_EXPORT void
dfftw_plan_guru_dft_omp_offload(PLAN *p, INTEGER *rank, INTEGER *n, INTEGER *is,
                    INTEGER *os, INTEGER *howmany_rank, INTEGER *howmany_n,
                    INTEGER *howmany_is, INTEGER *howmany_os, CFI_cdesc_t *in,
                    CFI_cdesc_t *out, INTEGER *sign, INTEGER *flags, void *interopObj)
{
    dfftw_plan_guru_dft_omp_offload_impl(p, rank, n, is, os, howmany_rank, howmany_n,
                                         howmany_is, howmany_os, (COMPLEX16 *)in->base_addr,
                                         (COMPLEX16 *)out->base_addr, sign,
                                         flags, interopObj);
}
...
#ifdef DFT_ENABLE_OFFLOAD
#ifdef MKL_ILP64
      *(fftw_plan *)p = fftw_plan_guru64_dft_omp_offload_impl_ilp64(*rank, dims64, *howmany_rank, howmany_dims64,
                                                   in, out, *sign, *flags, interopObj);
#else
      *(fftw_plan *)p = fftw_plan_guru64_dft_omp_offload_impl_lp64(*rank, dims64, *howmany_rank, howmany_dims64,
                                                   in, out, *sign, *flags, interopObj);
#endif
#else
    *(fftw_plan *)p =
