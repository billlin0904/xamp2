...
/*
 *
 * sfftw_plan_dft - FFTW3 Fortran 77 wrapper to Intel(R) oneAPI Math Kernel Library (Intel(R) oneMKL).
 *
 ******************************************************************************
 */
...
DLL_EXPORT void
sfftw_plan_dft_omp_offload(PLAN *p, INTEGER *rank, INTEGER *n, CFI_cdesc_t *in,
                 CFI_cdesc_t *out, INTEGER *sign, INTEGER *flags, void * interopObj)
{
    sfftw_plan_dft_omp_offload_impl(p, rank, n, (COMPLEX8 *)in->base_addr,
                                (COMPLEX8 *)out->base_addr, sign, flags, interopObj);
}

DLL_EXPORT void
sfftw_plan_dft_cpu(PLAN *p, INTEGER *rank, INTEGER *n, CFI_cdesc_t *in,
                 CFI_cdesc_t *out, INTEGER *sign, INTEGER *flags)
{
    sfftw_plan_dft(p, rank, n, (COMPLEX8 *)in->base_addr,
...
#ifdef MKL_ILP64
    *(fftwf_plan *)p =
        fftwf_plan_guru64_dft_omp_offload_impl_ilp64(*rank, dims64, 0, NULL, in,
                                                 out, *sign, *flags, interopObj);
#else
    *(fftwf_plan *)p =
        fftwf_plan_guru64_dft_omp_offload_impl_lp64(*rank, dims64, 0, NULL, in,
                                                 out, *sign, *flags, interopObj);
#endif
#else
    *(fftwf_plan *)p =
