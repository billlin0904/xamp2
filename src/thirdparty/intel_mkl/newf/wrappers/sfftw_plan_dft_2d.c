...
/*
 *
 * sfftw_plan_dft_2d - FFTW3 Fortran 77 wrapper to Intel(R) oneAPI Math Kernel Library (Intel(R) oneMKL).
 *
 ******************************************************************************
 */
...
DLL_EXPORT void
sfftw_plan_dft_2d_omp_offload(PLAN *p, INTEGER *nx, INTEGER *ny, CFI_cdesc_t *in,
                  CFI_cdesc_t *out, INTEGER *sign, INTEGER *flags, void *interopObj)
{
    INTEGER two = 2;
    INTEGER n[2] = { *nx, *ny };

    sfftw_plan_dft_omp_offload_impl(p, &two, n, (COMPLEX8 *)in->base_addr,
                                (COMPLEX8 *)out->base_addr, sign, flags, interopObj);
}

DLL_EXPORT void
sfftw_plan_dft_2d_cpu(PLAN *p, INTEGER *nx, INTEGER *ny, CFI_cdesc_t *in,
                                     CFI_cdesc_t *out, INTEGER *sign, INTEGER *flags)
{
    sfftw_plan_dft_2d(p, nx, ny, (COMPLEX8 *)in->base_addr,
